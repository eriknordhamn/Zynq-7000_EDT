...
int main(){
  ...
  */
  ...
  while(exit_flag != 1){
    ...
    print("Press '1' to use NORMAL GPIO as an input (BTN3 switch)\r\n");
    print("Press '2' to use EMIO as an input (BTN2 switch)\r\n");
    ...
    case '2' :
      //Usecase for PS GPIO
      exit_flag = 0;
      print("Press Switch 'BTN2' push button on board \r\n");
      print(" \r\n");
      while(internal_choice != '0'){
	Readstatus = XGpioPs_ReadPin(&psGpioInstancePtr,
				     iPinNumberEMIO);
	if(1== Readstatus && 0 == OldReadStatus ){
	  print("BTN2 PUSH Button pressed \n\r");
	  print("LED 'LD4' Turned OFF \r\n");
	  XGpioPs_WritePin(&psGpioInstancePtr,iPinNumber,0);
	  //Start Timer
	  XTmrCtr_Start(&TimerInstancePtr,0);
